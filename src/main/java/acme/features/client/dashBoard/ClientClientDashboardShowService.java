...
public class ClientClientDashboardShowService extends AbstractService<Client, ClientDashboard> {
	...
	public void load() {
		...
		double totalNumProgressLogBetween25and50 = progressLogsPublished.stream().filter(x -> myContractsIds.contains(x.getContract().getId())).filter(x -> x.getCompletenessPercentage() >= 25.0 && x.getCompletenessPercentage() <= 50.0).count();
		//PL between 50 and 75
		double totalNumProgressLogBetween50and75 = progressLogsPublished.stream().filter(x -> myContractsIds.contains(x.getContract().getId())).filter(x -> x.getCompletenessPercentage() > 50.0 && x.getCompletenessPercentage() <= 75.0).count();
		// PL above 75
		double totalNumProgressLogAbove75 = progressLogsPublished.stream().filter(x -> myContractsIds.contains(x.getContract().getId())).filter(x -> x.getCompletenessPercentage() > 75.0).count();
		//average
